...
- [`fable-prompting`](agent-workflows/fable-prompting/SKILL.md) — Router for prompting Fable so it performs like a next-generation model instead of a current one — give it the goal not the steps, hold it to a hard self-checkable bar, loop…
...
- [`humanizer`](agent-workflows/humanizer/SKILL.md) — Rewrite AI-sounding text so it reads like the writer without changing what it says.
...
- [`marketing`](agent-workflows/marketing/SKILL.md) — Router to the upstream Marketing Skills library by Corey Haines. Use for marketing and growth work of any kind, including copy and content, SEO and AI search, paid ads and ad…
...
#### emil
- [`animate`](design/emil/animate/SKILL.md) — Build an animation from scratch, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties,…
- [`animation-vocabulary`](design/emil/animation-vocabulary/SKILL.md) — Reverse-lookup glossary that turns a vague description of a web animation or motion effect into its exact term ("the bouncy thing when a popover opens" → Pop in; "the iOS…
- [`apple-design`](design/emil/apple-design/SKILL.md) — Opt-in reference for Apple's approach to interface design and fluid, physical motion, translated for the web, covering gesture-driven UI, springs, drag/swipe/sheet interactions,…
- [`emil-design-eng`](design/emil/emil-design-eng/SKILL.md) — Emil Kowalski's design-engineering philosophy for UI polish, component design, animation decisions, and the invisible details that make software feel great. Use for taste and…
- [`find-animation-opportunities`](design/emil/find-animation-opportunities/SKILL.md) — Search a codebase or UI for places that don't animate but should, and reject everything that shouldn't. Read-only; it proposes motion with exact values, it does not implement it.
- [`improve-animations`](design/emil/improve-animations/SKILL.md) — Survey a codebase's animation and motion code as a senior motion advisor, then produce a prioritized audit and self-contained implementation plans for other agents (or cheaper…
- [`mobile-native`](design/emil/mobile-native/SKILL.md) — Make a web app feel native on a phone — the small CSS and meta-tag fixes that separate "a website in a browser" from something that feels installed. Covers sticky hover states,…
- [`review-animations`](design/emil/review-animations/SKILL.md) — Reviews animation and motion code in a diff or focused surface against a high craft bar derived from Emil Kowalski's design engineering philosophy. Default to flagging; approval…

...
| **agent-interviewer** | Adaptive interview that generates a personalized behavior file for AI agents. Produces portable markdown (CLAUDE.md, AGENT.md, etc.) with concrete operating rules. Has a stricter V2 variant. | Generates your agent behavior profile instead of writing it from scratch. Run it once, use the output everywhere. Installed as `/interview` by wip-scaffold. |

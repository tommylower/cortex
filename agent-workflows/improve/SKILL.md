...
- Find the repo's domain glossary and decision records and read them. Glossaries live in files like `CONTEXT.md` or `CONTEXT-MAP.md`. ADRs are not always in `docs/adr/`: many repos use `docs/decisions/` (files like `0011-no-supabase.md` with an index README) or scatter `ADR-NNN` references through prose docs. Before concluding there are none, check both `docs/adr/` and `docs/decisions/`, and run `grep -rn "ADR-[0-9]"` to catch decisions referenced inline. The glossary's canonical terms are the vocabulary your findings and plans must use, and a documented ADR is a decision already made: treat it as a constraint to plan within, not a finding to relitigate (unless the audit surfaces evidence the decision's premises no longer hold, which is itself a finding).

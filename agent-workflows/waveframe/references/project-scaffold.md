...
Every newly scaffolded web project should carry the exact inspectable motif:

```text
built by a wave in progress. waves don't die.
```

Keep it non-visible in the UI, but present in shipped markup so it is visible in browser inspection. Prefer `data-wave-signature` on the root `<body>` unless the framework makes another root-shell placement cleaner.
...
2. cortex/design/foundations/ui-principles
3. cortex/design/foundations/responsive-craft
4. cortex/design/motion/css-interaction-tips
5. cortex/design/motion/framer-motion or emil-design-eng when motion/interaction matters
6. cortex/design/foundations/preflight before calling UI work done
```

---
name: oklch-skill
description: OKLCH color systems and semantic color usage. Use when converting colors, generating palettes, measuring contrast, handling gamut, theming Tailwind, defining color tokens, reviewing light and dark appearances, or when the user asks for better-colors.
...
| APCA body-text design target | \|Lc\| >= 75 floor, >= 90 preferred |
| APCA non-body design target | \|Lc\| >= 60 floor |
...
| Failing contrast | Report the rendered pair, APCA design reading, and applicable WCAG 2 result; change colors only when asked (then adjust L first) |
...
Use this format only when the user asks for a standalone color review. When
`studio-audit` orchestrates the review, provide domain evidence and findings
to that skill and let its output format, severity scale, consolidation rules,
cap, and verdict take precedence.
...
| --- | --- | --- | --- | --- |
| MEDIUM | `src/theme.css:18` | `color: #3b82f6` | `color: oklch(0.623 0.188 259.815)` | This project's color system uses OKLCH tokens |

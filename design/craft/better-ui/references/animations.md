...
All numeric values in this reference are fallback recipes. Use project motion
tokens first, then Studio's current defaults; tune rather than copying supplier
numbers into the product unchanged.
...
When the project has no contextual-icon recipe, start here and tune:
- `scale`: `0.25` → `1`
- `opacity`: `0` → `1`
- `filter`: `"blur(4px)"` → `"blur(0px)"`
- fallback transition: `{ type: "spring", duration: 0.3, bounce: 0 }`; use the
  project's established spring when one exists

## Scale on Press

A subtle scale-down on click gives buttons tactile feedback. Without a project
value, start at `scale(0.96)`; values below `0.95` usually feel exaggerated.
Use CSS transitions for interruptibility, so that if the user releases
mid-press, it smoothly returns.

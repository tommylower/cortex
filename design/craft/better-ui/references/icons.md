...
Preserve the project's icon system. Use this table only as a starting point
when no stroke language exists:
...
The swap between variants is a contextual icon animation; use the fallback
cross-fade recipe in [animations.md](animations.md) when the project has no
established version.

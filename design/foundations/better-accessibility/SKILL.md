...
description: Accessibility engineering for product interfaces. Use when building or reviewing semantics, keyboard and focus behavior, ARIA, forms, screen-reader output, hit areas, zoom, or reduced motion.
...
Rendered-pair contrast measurement and color remediation are covered by
`oklch-skill`; visual text sizing and iOS input zoom by
`better-typography`; spatial RTL layout by `better-layout`.
...
Use this format only when the user asks for a standalone accessibility review.

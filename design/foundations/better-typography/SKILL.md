...
description: Web typography from font choice through rendered text behavior. Use when building or reviewing type systems, font loading and features, hierarchy, spacing, wrapping, truncation, punctuation, or text accessibility.
...
The words themselves (button labels, error messages, empty states) are covered by the `better-writing` skill; semantic heading structure by `better-accessibility`; spatial RTL layout and logical CSS properties by `better-layout`; rendered-pair contrast measurement and color remediation by `oklch-skill`. This skill owns how text renders, wraps, and behaves in mixed-direction content.
...
`text-wrap: balance` distributes short text evenly across lines: use it on
headings and short descriptions when the intended result is an even rag.
Chromium ignores it after roughly six lines, so it is safe but ineffective on
long-form text. Use `text-wrap: pretty` on known long-form copy when the goal is
preventing a single-word final line; it does not repair every short two-word
rag. Use `overflow-wrap: break-word` where long words, links, or IDs could
escape the container, and `white-space: nowrap` where a label or badge should
never break.
...
| Single-word orphan in known long-form copy | `text-wrap: pretty` |
| Lopsided heading or short description | `text-wrap: balance` |
...
Use this format only when the user asks for a standalone typography review.

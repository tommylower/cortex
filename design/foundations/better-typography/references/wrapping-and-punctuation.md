...
Use `balance` on headings and short descriptions when the goal is an even rag.
Chromium ignores it after roughly six lines. Use `pretty` on known long-form
copy when the goal is avoiding a single-word final line; it does not fix every
short two-word rag.

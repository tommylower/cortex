#!/usr/bin/env bash
# Configure local Claude and Codex installs to use cortex skills and
...
start_commands = [
    ...
    f"{cortex_root}/scripts/session-journal-start.sh \"$PWD\" >/dev/null 2>&1 || true",
]
...
stop_group = normalize_group(stop_entries, "")
stop_hooks = stop_group["hooks"]
stop_command = f"{cortex_root}/scripts/session-journal-stop.sh \"$PWD\" >/dev/null 2>&1 || true"
if not any(isinstance(h, dict) and h.get("command") == stop_command for h in stop_hooks):
    stop_hooks.append({"type": "command", "command": stop_command})
...
setup_codex() {
  ...
  CODEX_CONFIG_PATH="$config_path" CORTEX_ROOT="$CORTEX_ROOT" python3 <<'PY'
import os
import re
from pathlib import Path
...
cortex_root = os.environ["CORTEX_ROOT"]
wrapper = f"{cortex_root}/scripts/codex-turn-ended-notify.sh"
...
if match:
    ...
    new_items = [wrapper] + [item for item in existing if item != wrapper]
    replacement = 'notify = [' + ', '.join(f'"{item}"' for item in new_items) + ']'
    suffix = text[match.end():]
    if suffix and not suffix.startswith("\n"):
        replacement += "\n"
    text = text[:match.start()] + replacement + suffix
else:
    prefix = f'notify = ["{wrapper}"]\n'

...
- keep local process notes in \`.waveframe/\`
...
description: Apply a reusable output template, such as a design-system deck, brand overview deck, product UI system board, landing-page variant, or dashboard structure, to a selected design system and target surface.
...
Ask which reusable output template to use, which design system to apply, and which target surface to build in. Produce an application plan and ask approval before writing."
